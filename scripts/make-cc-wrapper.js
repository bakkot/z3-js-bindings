...
for (let fnName of asyncFns) {
  ...
  wrappers.push(`
extern "C" void async_${fn.name}(int promise_idx, ${fn.params.map(p => `${p.isConst ? 'const ' : ''}${p.type}${p.isPtr ? '*' : ''} ${p.name}${p.isArray ? '[]' : ''}`).join(', ')}) {
  wrapper<decltype(&${fn.name}), &${fn.name}>(promise_idx, ${fn.params.map(p => `${p.name}`).join(', ')});
}
`.trim());
}
